    ...
    with torch.no_grad():
        for i in range(length):
            if i == 0:
                ...
                one_hot = chars_to_onehot(char_to_add, char_to_idx)
            one_hot_3dim = one_hot.unsqueeze(0).float().to(device)
            if i == 0:
                y, h = model(one_hot_3dim)
            else:
                y, h = model(one_hot_3dim, h)
            if i == 0:
                y_use = y[0][-1]
            else:
                y_use = y[0][0]
